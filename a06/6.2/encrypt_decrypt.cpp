...
void trivialEncryption::encrypt()
{
    std::string blockEncrypted = this->data;
    for(int i = 0; i < blockEncrypted.size(); i++)
    {
        this->BlockCyphers.push_back(substitutionCipher(blockEncrypted[i], this->key));
...
void trivialEncryption::decrypt()
{
    std::string blockDecrypted = this->data;
...
trivialEncryption::trivialEncryption(std::string newData, int newKey){
    data = newData;
    key = newKey;
...
std::string trivialEncryption::getData()
{
    return this->data;
}
